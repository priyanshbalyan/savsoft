...
function open_quiz($limit='0'){
	...
	public function validate_quiz($quid){
		...
		$userdata=array(
		'email'=>time(),
		'password'=>pasword_hash(rand(11111,99999), PASSWORD_BCRYPT),
 ...
 function submit_quiz(){
	 ...
	 $rid=$this->session->userdata('rid');
				...
				if($this->quiz_model->submit_result()){
					...
					$this->session->set_flashdata('message', "<div class='alert alert-success'>".json_encode($user_verify_detail)." ".json_encode($user)."</div>");
